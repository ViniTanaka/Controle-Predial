...
public class CryptoDummy
{
	...
	public void geraChave(File fDummy) throws IOException
	{
		...
		ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream("chave.dummy"));
		oos.writeObject(dk);
		oos.close();
	}
		...
		throws IOException, ClassNotFoundException
	{
		...
		{
			textoCifrado[i] = (byte) (textoCifrado[i] + 2);
		...
		throws IOException, ClassNotFoundException
	{
		...
		for(int i = 0; i < texto.length; i++)
		{
			textoDecifrado[i] = (byte) (textoDecifrado[i] - 2);
		}
	}
        
//    public void geraDecifra2(byte[] texto, File fDummy)
//		throws IOException, ClassNotFoundException
//	{
